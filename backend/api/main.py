...
HOSPITALS: List[Dict] = [
    ...
    {"id": "h2", "name": "Yashodhara Hospital",      "lat": 17.6800, "lon": 75.9300, "total_beds": 60,  "beds_available": 48, "icu_beds": 10, "icu_available": 7,  "wait_time": 5,  "address": "Central Sector"},
    {"id": "h3", "name": "Monark Hospital",          "lat": 17.6200, "lon": 75.8700, "total_beds": 120, "beds_available": 91, "icu_beds": 30, "icu_available": 22, "wait_time": 20, "address": "South-West Sector"},
    {"id": "h4", "name": "Civil Hospital",           "lat": 17.6300, "lon": 75.9800, "total_beds": 90,  "beds_available": 63, "icu_beds": 25, "icu_available": 18, "wait_time": 8,  "address": "South-East Sector"},
]

AMBULANCES: List[Dict] = [
    {"id": "a0", "name": "AMB-001", "lat": 17.7000, "lon": 75.9000, "status": "available", "assigned_patient": None, "target_hosp_id": None, "speed_kmh": 60},
    {"id": "a1", "name": "AMB-002", "lat": 17.6500, "lon": 75.9500, "status": "available", "assigned_patient": None, "target_hosp_id": None, "speed_kmh": 65},
    {"id": "a2", "name": "AMB-003", "lat": 17.7200, "lon": 75.9600, "status": "available", "assigned_patient": None, "target_hosp_id": None, "speed_kmh": 55},
    {"id": "a3", "name": "AMB-004", "lat": 17.6400, "lon": 75.8900, "status": "available", "assigned_patient": None, "target_hosp_id": None, "speed_kmh": 70},
]
...
def get_live_tracking():
    return {"hospitals": HOSPITALS, "ambulances": AMBULANCES, "patients": list(PATIENTS.values())}
...
async def simulation_loop():
    ...
    while simulation_running:
        ...
        if free_ambs and random.random() < 0.075: 
            fake = PatientIn(name=f"Sim-{random.randint(100,999)}", severity=random.uniform(1,10), lat=random.uniform(LAT_MIN, LAT_MAX), lon=random.uniform(LON_MIN, LON_MAX))
            try: add_patient(fake)
            except: pass

        for h in HOSPITALS:
            if random.random() < 0.005 and h["beds_available"] < h["total_beds"]: 
                h["beds_available"] += 1
